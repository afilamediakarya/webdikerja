...
(glob.sync("resources/plugins/custom/**/*.js") || []).forEach((file) => {
    mix.js(
        file,
        `public/${file.replace("resources/", "").replace(".js", ".bundle.js")}`
    );
});
...
(glob.sync("resources/metronic/sass/pages/**/!(_)*.scss") || []).forEach(
    (file) => {
        ...
        mix.sass(
            file,
            file
                .replace("resources/metronic/sass", "public/css")
                .replace(/\.scss$/, ".css")
        );
    }
);
...
(
    glob.sync(
        "node_modules/+(@fortawesome|socicon|line-awesome)/**/*.+(woff|woff2|eot|ttf)"
    ) || []
).forEach((file) => {
