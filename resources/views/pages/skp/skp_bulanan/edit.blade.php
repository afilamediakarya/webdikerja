                ...
                <div class="card-body">
                    ...
                    <form class="form" id="skp-form">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" type="text" disabled name="rencana_kerja" id="rencana_kerja">
                                <option selected disabled>Pilih Sasaran Kinerja</option>
                                @foreach($get_sasaran_kinerja as $key => $value)
                                    <option value="{{$value['id']}}" @if($data['id'] == $value['id']) selected @endif>{{$value['value']}}</option>
                                @endforeach
                             </select>
                        ...
                        <div class="row">
                            <div class="col-lg-8">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" disabled name="indikator_kerja_individu[{{$key}}]" id="indikator_kerja_individu_{{$key}}" rows="5">{{ $value['iki'] }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" disabled value="{{$value['satuan']}}" name="satuan[{{$key}}]">
                            </div>
                            @foreach($value['target_skp'] as $index => $val)
                                @if($val['bulan'] == $bulan)
                                <input type="hidden" name="id_target[{{$key}}]" value="{{$val['id']}}">
                                <div class="form-group">
                                    <label>Target bulan {{$bulan}}</label>
                                    <input type="text" class="form-control" value="{{$val['target']}}" name="target[{{$key}}]" id="target">
